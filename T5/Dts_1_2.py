...
for j in range(20):
    ...
    for g1, g_param in enumerate(GRID):
        ...
        for train, test in slices.split(oData.Training_indexes):
            ...
            mlpsk = MLPRegressor(hidden_layer_sizes=( g_param, ), activation='logistic', solver='adam', learning_rate='constant', learning_rate_init=LEARNING_RATE, verbose=True)
    ...
    for coisa in range(360):
        plt.scatter(coisa, oData.model.predict([[coisa]])[0], color='red')
        plt.scatter(coisa, 3 * np.sin(coisa * np.pi / 180) + 1, color='green')
    plt.show()
