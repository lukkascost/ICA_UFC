...
for i in range(base.shape[0]):
    classe = np.zeros(len(LE.classes_))
    classes[LE.transform([classes[i]])[0]] = 1
    pattern = Pattern(i + 1, base[i], classe)
    patterns.append(pattern)
classLabels = list(LE.classes_.astype(str))
data = Data(patterns, classLabels)
rbf = RBFNetwork(base.shape[1], 10, len(classLabels), data)
mse = rbf.train(100)
